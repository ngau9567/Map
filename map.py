...
agree = st.checkbox('Art')
agree_1 = st.checkbox('Culture')

if agree:
    list_art = data[data['Subject'].str.contains('Art')]
    st.write(f"Collection Selected by Art: {len(list_art)}")
    st.write(list_art)

if agree_1:
    list_cul = data[data['Subject'].str.contains('Culture')]
    st.write(f"Collection Selected by Culture: {len(list_cul)}")
    st.write(list_cul)
